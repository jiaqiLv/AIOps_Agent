"""
ZH Dataset Causal Discovery Runner

Simplified pipeline for ZH dataset (single sample, no evaluation).

Usage:
    python -m run_zh --sample 0105
    python -m run_zh --config config/zh_config.yaml
"""
...
def _build_graph_from_dataframe(causal_graph: pd.DataFrame):
...
def orient_graph(config_manager: ConfigManager, algorithm_results,
    ...
    # Convert CausalGraph to DataFrame
...
def load_zh_data(config_manager: ConfigManager, logger: logging.Logger) -> DataContainer:
    """Load ZH dataset from CSV file and root_cause.txt."""
    logger.info("=" * 60)
    ...
    # Load root_cause.txt for ZH dataset (same format as RADICE)
    sample_path = Path(container.metadata.get('source_path')).parent
    root_cause_path = sample_path / 'root_cause.txt'
    ...
    if root_cause_path.exists():
        ...
        parts = content.split()
        root_causes = [str(parts[0])] if len(parts) >= 1 else []
        symptoms = [str(parts[1])] if len(parts) >= 2 else []

        container.metadata['ground_truth'] = {
        ...
        logger.warning(f"root_cause.txt not found: {root_cause_path}")

    return container


def generate_constraints_zh(config_manager: ConfigManager, container: DataContainer,
                            logger: logging.Logger) -> tuple:
    """Generate knowledge constraints (ZH dataset has no layer.txt)."""
    logger.info("=" * 60)
    ...
    # ZH dataset: only use YAML configuration (no layer.txt)
    if use_level_constraints:
        level_map = knowledge_manager.get_level_map(metric_names)
...
def run_rca_zh(config_manager: ConfigManager, data: pd.DataFrame,
               container: DataContainer, causal_graph: pd.DataFrame,
               level_map: dict, logger: logging.Logger) -> dict:
    """Run root cause analysis for ZH dataset."""
    logger.info("=" * 60)
    ...
    # Determine performance metric: root_cause.txt symptom > config > first column
    ...
    if gt and 'symptoms' in gt and gt['symptoms']:
        ...
        if symptom_id in data.columns:
            ...
            logger.info(f"Using performance metric from root_cause.txt: {performance_metric}")
        else:
    ...
    if not performance_metric:
        ...
        logger.info(f"Using first column as performance metric: {performance_metric}")
...
def save_results_zh(config_manager: ConfigManager, processed_data: pd.DataFrame,
                    algorithm_results, oriented_graph: pd.DataFrame,
                    orientation_metadata: dict, rca_results: dict,
                    logger: logging.Logger) -> None:
    """Save results (ZH dataset: no evaluation)."""
    logger.info("=" * 60)
    ...
    correlation_matrix = processed_data.corr(method='pearson') if processed_data is not None else None
    ...
    saver.save_all(
        ...
        eval_results=None,  # No evaluation for ZH
...
def main_zh(config_path: str = "config/zh_config.yaml"):
    """Main entry point for ZH dataset."""
    ...
    logger = setup_logger(level=log_level, log_file=log_file, format_string=log_format)

    logger.info("=" * 60)
    logger.info("ZH Dataset Causal Discovery Pipeline")
    logger.info("=" * 60)
    logger.info(f"Configuration: {config_path}")
    logger.info(f"Dataset: {config_manager.get('context.dataset_name')}")
    logger.info(f"Sample: {config_manager.get('context.sample_name')}")
    ...
    try:
        ...
        container = load_zh_data(config_manager, logger)
        ...
        # Step 3: Constraints
        step_start = time.time()
        background_knowledge, level_map = generate_constraints_zh(config_manager, container, logger)
        ...
        oriented_graph, orientation_metadata = orient_graph(
            config_manager, algorithm_results, processed_data, logger
        )
        step_times['orientation'] = time.time() - step_start

        # Step 6: RCA
        step_start = time.time()
        rca_results = run_rca_zh(
        ...
        save_results_zh(
            ...
            oriented_graph, orientation_metadata, rca_results, logger
        )
        ...
        logger.info(f"  Step 7 - Save Results:       {step_times.get('save', 0):.2f}s")
        logger.info("-" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ZH Dataset Causal Discovery Pipeline")
    parser.add_argument("--config", type=str, default="config/zh_config.yaml",
                        help="Path to configuration file")
    parser.add_argument("--sample", type=str,
                        help="Sample name (overrides config file)")

    args = parser.parse_args()

    if args.sample:
        # Update sample in config
        import yaml
        config_path = args.config
        with open(config_path, 'r') as f:
            config = yaml.safe_load(f)
        config['context']['sample_name'] = args.sample

        temp_config = config_path.replace('.yaml', f'_{args.sample}.yaml')
